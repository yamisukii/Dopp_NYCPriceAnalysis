    ...
    for column in column_names:
        if column.endswith('DT') or column.endswith('DATE') or column.endswith('date'):
            df[column] = pd.to_datetime(
                df[column], errors='coerce', format='%m/%d/%Y')
    return df
...
def occurences_per_date_column(df, column_name):
    temp = df.copy()
    temp['year_month'] = temp[column_name].dt.to_period(
        'M')  # Convert to 'YYYY-MM' format
    monthly_counts = temp['year_month'].value_counts(
    ).sort_index()  # Count and sort by time

    plt.figure(figsize=(12, 6))
    monthly_counts.plot(kind='bar', color='skyblue')
    plt.title(
        f'Occurrences per Month Over the Years for {column_name}', fontsize=16)
...
def count_unique_values(df, column_name):
    ...
    counts.head(top_n).plot(kind='bar', figsize=(
        12, 6), color='skyblue', edgecolor='black')

    plt.title(f'Count unique values for {column_name}'.format(
        top_n), fontsize=16)
...
def id_off(df, column_name_one, column_name_two):
    ...
    unique_combinations = df[[column_name_one,
                              column_name_two]].drop_duplicates()

    # Count how many unique combinations there are
    num_unique_combinations = unique_combinations.shape[0]
    print(
        f'Number of unique combinations of {column_name_one} and {column_name_two}: {num_unique_combinations}')
...
def handle_police_data(file_path, beginning_year):
    ...
    df = drop_na_location(
        df, ['Latitude', 'Longitude', 'Lat_Lon', 'New Georeferenced Column'])
...
# For police data
file_path = '../data/police_data/NYPD_Complaint_Data_Current__Year_To_Date__20241223.csv'
